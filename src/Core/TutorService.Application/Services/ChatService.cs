...
public class ChatService : IChatService
{
    ...
    public async Task<ChatsResponse> GetUserChatsAsync(Guid userId, int page = 1, int pageSize = 20)
    {
        var tutorProfile = await _tutorProfileRepository.GetByUserIdAsync(userId);
        ...
        var chats = await _chatRepository.GetUserChatsAsync(userId, page, pageSize);
        var totalCount = await _chatRepository.GetUserChatsCountAsync(userId);
    ...
    public async Task<MessagesResponse> GetChatMessagesAsync(Guid chatId, Guid userId, int page = 1, int pageSize = 20, Guid? beforeMessageId = null)
    {
        var tutorProfile = await _tutorProfileRepository.GetByUserIdAsync(userId);
        ...
        var messages = await _messageRepository.GetChatMessagesAsync(chatId, page, pageSize, beforeMessageId);
        var totalCount = await _messageRepository.GetChatMessagesCountAsync(chatId);
