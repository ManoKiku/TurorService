...
public class ChatsController : ControllerBase
{
    ...
    [HttpGet("my-chats")]
    public async Task<ActionResult<ChatsResponse>> GetMyChats(
        [FromQuery] int page = 1,
        [FromQuery] int pageSize = 20)
    {
        var userId = ControllerHelper.GetUserIdFromClaims(User);;
        var response = await _chatService.GetUserChatsAsync(userId, page, pageSize);
        return Ok(response);
    }
    ...
    public async Task<ActionResult<MessagesResponse>> GetChatMessages(
        Guid chatId,
        [FromQuery] int page = 1,
        [FromQuery] int pageSize = 20,
    ...
    {
        var userId = ControllerHelper.GetUserIdFromClaims(User);;
        var response = await _chatService.GetChatMessagesAsync(chatId, userId, page, pageSize, beforeMessageId);
        return Ok(response);
}
